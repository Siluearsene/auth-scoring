...
class UserService:
    ...
    def create_user(self, user_data: UserCreate) -> User:
        existing_user = self.user_repo.get_user_by_matricule(user_data.matricule)
        if existing_user:
    ...
    def get_user_by_matricule(self, matricule: str) -> Optional[User]:
        """
        ...
        :return: Utilisateur trouvé ou None
        """
        return self.user_repo.get_user_by_matricule(matricule)
    ...
    def authenticate_user(self, matricule: str, password: str) -> User:
        ...
        """
        try:
            user = self.user_repo.get_user_by_matricule(matricule)
            if not user:
                logfire.warn(
                    f"Échec de la connexion : utilisateur avec matricule {matricule} non trouvé.")
                raise InvalidCredentialsException()
            if not verify_password(password, user.password):
                logfire.warn(
                    f"Échec de la connexion : mot de passe incorrect pour l'utilisateur avec matricule {matricule}.")
                raise InvalidCredentialsException()
            logfire.info(
                f"Connexion réussie pour l'utilisateur avec matricule {matricule}.")
            return user
        except Exception as e:
            logfire.error(
                f"Erreur lors de l'authentification pour l'utilisateur avec matricule {matricule}: {str(e)}")
            raise
